...
def process_single_video(file_path, options, app):
    ...
    if options.get('convert_video') or options.get('remove_audio') or (options.get('rotate_option') and options.get('rotate_option') != "None"):
...
def extract_frames_from_video(file_path, interval, app):
    ...
    cmd = [
        "ffmpeg", "-i", file_path,
        "-vf", f"select='not(mod(n,{interval}))',setpts=N/TB",
        "-vsync", "vfr",
        "-f", "image2",
        out_pattern
    ]
...
def extract_gif_from_video(file_path, app):
    ...
    cmd = [
        "ffmpeg", "-i", file_path,
        "-vf", "fps=10,scale=480:-1:flags=lanczos", "-gifflags", "+transdiff",
        gif_file
    ]
