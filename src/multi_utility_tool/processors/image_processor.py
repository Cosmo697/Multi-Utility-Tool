    ...
    try:
        ...
        if resize_method != "none":
            if resize_method == "percentage":
                ...
                img = resize_image_fixed(
                    img, fixed_width, fixed_height, maintain_aspect, aspect_choice
                )
        if margin:
        ...
        new_base = (
            f"{rename_prefix}_{base_name}" if do_rename else base_name + "_processed"
        )
...
def add_image_inner_margin(img):
    ...
    new_img.paste(
        img.crop((margin, margin, width - margin, height - margin)), (margin, margin)
    )
    return new_img
