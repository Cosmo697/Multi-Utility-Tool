...
def generate_unique_file_path(output_folder, base_name, suffix, ext):
    ...
    while os.path.exists(potential_file_path):
        ...
        potential_file_path = os.path.join(
            output_folder, f"{base_name}{suffix}_{padded_counter}.{ext}"
        )
        counter += 1
...
def create_drop_area(
    parent, plugin_api=None, width=40, height=10, text_str="Drop files here"
):
    ...
    class DropArea(tk.Text):
        def __init__(self, master):
            super().__init__(
                master, width=width, height=height, bg="#3e3e3e", fg="#d3d3d3"
            )
            ...
            self.menu.add_command(
                label="Open File Location", command=self.open_location
            )
    ...
    if ext == ".txt":
        ...
        with open(file_path, "rb") as f:
            try:
                ...
                pages = reader.pages
            except AttributeError:  # fallback for older PyPDF2 versions
                reader = PyPDF2.PdfFileReader(f)
                pages = [reader.getPage(i) for i in range(reader.numPages)]

            texts = []
            for page in pages:
                extract = getattr(page, "extract_text", None) or getattr(
                    page, "extractText", None
                )
                if extract:
                    texts.append(extract() or "")
            return "\n".join(texts)
    else:
