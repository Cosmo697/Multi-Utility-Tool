...
from ..processors.video_processor import join_multiple_clips, process_videos
...
def register_plugin(plugin_api):
    ...
    ttk.Label(opts_frame, text="Extract every nth frame:").grid(
        row=0, column=0, sticky=tk.W
    )
    ttk.Entry(opts_frame, textvariable=frame_interval_var, width=5).grid(
        row=0, column=1, padx=5
    )
    extract_frames_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(
        opts_frame, text="Extract Frames", variable=extract_frames_var
    ).grid(row=0, column=2, sticky=tk.W, padx=5)

    extract_audio_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(opts_frame, text="Extract Audio", variable=extract_audio_var).grid(
    ...
    ttk.Checkbutton(
        opts_frame, text="Remove Audio Track", variable=remove_audio_var
    ).grid(row=2, column=0, sticky=tk.W)

    join_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(opts_frame, text="Join Multiple Clips", variable=join_var).grid(
        row=3, column=0, sticky=tk.W
    )

    convert_video_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(
        opts_frame, text="Convert Video Format", variable=convert_video_var
    ...
    ttk.Combobox(
        opts_frame,
        textvariable=convert_format_var,
    ...
    ttk.Checkbutton(
        opts_frame, text="Keep Original Quality", variable=keep_quality_var
    ).grid(row=5, column=0, sticky=tk.W)
    # Now working in Mbps
    ttk.Label(opts_frame, text="Target Bitrate (Mbps):").grid(
    ...
    ttk.Checkbutton(opts_frame, text="Extract GIF", variable=extract_gif_var).grid(
    ...
    ttk.Combobox(
        opts_frame,
        textvariable=rotate_option_var,
    ...
    drop_area = create_drop_area(
        drop_frame, plugin_api, text_str="Drag & drop video files/folders here"
    )
            ...
            if os.path.isdir(path):
                video_files.extend(
                    find_files_in_folder(
                        path, valid_extensions=VALID_EXTENSIONS["VIDEO"]
                    )
                )
