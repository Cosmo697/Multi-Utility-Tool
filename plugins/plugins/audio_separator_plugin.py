"""Audio Separator plugin for stem separation and voice denoise."""
...
class AudioSeparatorUI:
    ...
    def _build_ui(self):
        ...
        self._build_demucs_tab()
        self._build_dpf_tab()

    def _build_demucs_tab(self):
        f = self.demucs_tab
    ...
    def browse(self, var, mode, types=None):
        if mode == "file":
            val = filedialog.askopenfilename(filetypes=types or [("All files", "*.*")])
        else:
    ...
    def run_dpf(self):
        ...
        def chunk_process():
            ...
            self.console_dpf.insert(tk.END, f"\U0001F4C2 Processing {nchunks} chunks from {inp}\n\n")
...
def register_plugin(plugin_api):
    ...
    plugin_api.add_tab("Audio Separator", tab)
    AudioSeparatorUI(tab, plugin_api)
