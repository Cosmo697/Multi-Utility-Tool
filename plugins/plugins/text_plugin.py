...
def register_plugin(plugin_api):
    ...
    ttk.Checkbutton(opts_frame, text="Merge Files", variable=merge_var).grid(
        row=0, column=0, sticky=tk.W
    )
    ttk.Checkbutton(
        opts_frame, text="Deduplicate Words", variable=deduplicate_var
    ...
    ttk.Entry(opts_frame, textvariable=replace_var, width=15).grid(
        row=2, column=1, padx=5
    )
    ttk.Checkbutton(
        opts_frame, text="Convert MD to HTML/PDF", variable=convert_md_var
    ).grid(row=3, column=0, sticky=tk.W)
    ttk.Checkbutton(
        opts_frame, text="Word Freq & Stats (WordCloud)", variable=freq_stats_var
    ...
    drop_area = create_drop_area(
        drop_frame, plugin_api, text_str="Drag & drop text files/folders here"
    )
            ...
            if os.path.isdir(path):
                text_files.extend(
                    find_files_in_folder(
                        path, valid_extensions=VALID_EXTENSIONS["TEXT"]
                    )
                )
    ...
    def display_content(content):
        ...
        ttk.Button(button_frame, text="Undo", command=text_widget.edit_undo).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(button_frame, text="Redo", command=text_widget.edit_redo).pack(
            side=tk.LEFT, padx=2
        )
