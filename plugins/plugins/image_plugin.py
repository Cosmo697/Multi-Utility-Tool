...
def register_plugin(plugin_api):
    ...
    none_rb = ttk.Radiobutton(resize_options_frame, text="No Resizing", variable=resize_method_var, value="none")
    percentage_rb = ttk.Radiobutton(resize_options_frame, text="By Percentage", variable=resize_method_var, value="percentage")
    fixed_rb = ttk.Radiobutton(resize_options_frame, text="Fixed Dimensions", variable=resize_method_var, value="fixed")
    ...
    ttk.Label(resize_options_frame, text="Resize %:").grid(row=1, column=0, sticky=tk.W, padx=5)
    resize_percentage_var = tk.IntVar(value=50)
    percentage_entry = ttk.Entry(resize_options_frame, textvariable=resize_percentage_var, width=5)
    percentage_entry.grid(row=1, column=1, padx=5, pady=2, sticky=tk.W)

    ttk.Label(resize_options_frame, text="Width:").grid(row=2, column=0, sticky=tk.W, padx=5)
    fixed_width_var = tk.IntVar(value=800)
    fixed_width_entry = ttk.Entry(resize_options_frame, textvariable=fixed_width_var, width=7)
    fixed_width_entry.grid(row=2, column=1, padx=5, pady=2, sticky=tk.W)

    ttk.Label(resize_options_frame, text="Height:").grid(row=3, column=0, sticky=tk.W, padx=5)
    fixed_height_var = tk.IntVar(value=600)
    fixed_height_entry = ttk.Entry(resize_options_frame, textvariable=fixed_height_var, width=7)
    fixed_height_entry.grid(row=3, column=1, padx=5, pady=2, sticky=tk.W)

    maintain_aspect_var = tk.BooleanVar(value=False)
    aspect_check = ttk.Checkbutton(resize_options_frame, text="Maintain Aspect Ratio", variable=maintain_aspect_var)
    aspect_check.grid(row=4, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)

    ttk.Label(resize_options_frame, text="Aspect Ratio:").grid(row=5, column=0, sticky=tk.W, padx=5)
    aspect_choice_var = tk.StringVar(value="Original")
    aspect_options = ["Original", "1:1", "4:3", "16:9", "3:2"]
    aspect_combo = ttk.Combobox(resize_options_frame, textvariable=aspect_choice_var, values=aspect_options, state="readonly", width=10)
    ...
    ttk.Checkbutton(opts_frame, text="Add 16px Margin", variable=margin_var).grid(row=0, column=0, sticky=tk.W)
    ttk.Checkbutton(opts_frame, text="Batch Rename", variable=batch_rename_var).grid(row=1, column=0, sticky=tk.W)
    ttk.Label(opts_frame, text="Rename Prefix:").grid(row=1, column=1, sticky=tk.W)
    ttk.Entry(opts_frame, textvariable=rename_prefix_var, width=12).grid(row=1, column=2, padx=5)
    ttk.Checkbutton(opts_frame, text="Reformat Image", variable=reformat_var).grid(row=2, column=0, sticky=tk.W)
    ttk.Label(opts_frame, text="New Format:").grid(row=2, column=1, sticky=tk.W)
    ttk.Combobox(opts_frame, textvariable=new_format_var, values=["jpg", "png", "bmp", "gif"], state="readonly", width=5).grid(row=2, column=2, padx=5)
    ...
    def image_drop_event(e):
        ...
        for path in paths:
            if os.path.isdir(path):
                image_files.extend(find_files_in_folder(path, valid_extensions=VALID_EXTENSIONS["IMAGE"]))
        ...
        t = threading.Thread(
            ...
            args=(image_files, resize_method_var.get(), resize_percentage_var.get(),
                  fixed_width_var.get(), fixed_height_var.get(), maintain_aspect_var.get(),
                  aspect_choice_var.get(), margin_var.get(), batch_rename_var.get(),
                  rename_prefix_var.get(), reformat_var.get(), new_format_var.get(), plugin_api.app),
            daemon=True
        )
        t.start()
