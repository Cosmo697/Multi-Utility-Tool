...
def register_plugin(plugin_api):
    ...
    ttk.Radiobutton(tab, text="Compress", variable=action_var, value="compress").grid(
        row=0, column=0, sticky=tk.W, padx=5
    )
    ttk.Radiobutton(tab, text="Extract", variable=action_var, value="extract").grid(
        row=0, column=1, sticky=tk.W, padx=5
    )

    name_var = tk.StringVar(value="archive")
    ttk.Label(tab, text="Output Name/Folder:").grid(
    ...
    drop_area = create_drop_area(
        drop_frame, plugin_api, text_str="Drag & drop files/folders here"
    )
