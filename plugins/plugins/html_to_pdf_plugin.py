...
def render_page_to_pdf(chrome_path, url, output_path, cancel_event):
    process = subprocess.Popen(
        [chrome_path, "--headless", "--disable-gpu", f"--print-to-pdf={output_path}", url],
                ...
                try:
                    async with session.get(url, timeout=10) as response:
                        if response.status != 200:
                            log_callback(f"Warning: {url} returned status code {response.status}.")
                            queue.task_done()
                            continue
...
async def async_render_pages(urls, chrome_path, temp_dir, cancel_event, log_callback, progress_update_callback):
    ...
    async def render_page(idx, page_url):
        ...
        async with semaphore:
            await loop.run_in_executor(None, render_page_to_pdf, chrome_path, page_url, output_file, cancel_event)
...
async def generate_pdf_async(url, output_path, chrome_path, log_callback,
                             progress_init_callback, progress_update_callback, cancel_event):
    ...
    try:
        pdf_files = await async_render_pages(urls, chrome_path, temp_dir, cancel_event, log_callback, progress_update_callback)
        if cancel_event.is_set():
...
class HtmlToPdfPlugin(ttk.Frame):
    ...
    def build_ui(self):
        ...
        ttk.Label(self, text="Documentation URL:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.url_entry = ttk.Entry(self, width=60)
        self.url_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(self, text="Output PDF Filename:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.out_entry = ttk.Entry(self, width=60)
        self.out_entry.grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(self, text="Chrome Path:").grid(row=2, column=0, sticky="e", padx=5, pady=5)
        self.chrome_entry = ttk.Entry(self, width=60)
        self.chrome_entry.insert(0, r"C:\Program Files\Google\Chrome\Application\chrome.exe")
        self.chrome_entry.grid(row=2, column=1, padx=5, pady=5)

        self.button_frame = ttk.Frame(self)
        self.button_frame.grid(row=3, column=0, columnspan=2, pady=10)
        
        self.generate_button = ttk.Button(self.button_frame, text="Generate PDF", command=self.on_generate)
        self.generate_button.grid(row=0, column=0, padx=5)
        
        self.cancel_button = ttk.Button(self.button_frame, text="Cancel", command=self.on_cancel, state="disabled")
        self.cancel_button.grid(row=0, column=1, padx=5)
        
        self.open_dir_button = ttk.Button(self.button_frame, text="Open Output Directory", command=self.open_output_directory, state="disabled")
    ...
    def on_generate(self):
        ...
        if not os.path.isfile(chrome_path):
            messagebox.showerror("Error", f"Chrome executable not found at {chrome_path}")
            return
        ...
        threading.Thread(target=self.run_generate, args=(url, out_filename, chrome_path), daemon=True).start()
    ...
    def run_generate(self, url, out_filename, chrome_path):
        try:
            asyncio.run(generate_pdf_async(
                url, out_filename, chrome_path, self.log_message,
                self.progress_init, self.progress_update, self.cancel_event))
